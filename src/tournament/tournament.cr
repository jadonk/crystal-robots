...
module CrystalRobots::Tournament
  ...
  # total (the same shared budget a pool pairing gets, not a per-game
  # retry on top of a per-game retry), stopping as soon as either entrant
  # reaches 2 wins. A match still tied (including 0-0, every attempt a
  # no-winner) after that budget is spent falls back to the first-listed
  # (higher-seeded) entrant, so a match can never balloon into a dozen
  # refights the way independent per-game retries could.
