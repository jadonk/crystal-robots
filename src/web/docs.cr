# API documentation, built by `crystal-robots build-docs` (which runs
# `crystal docs` into `docs-api/`, an ignored directory) and embedded
# into the binary at compile time.
#
# The generated pages are not served as they are: their own UI needs
# an inline script that Fossil's content security policy blocks.
# Instead each page's body is extracted, its scripts and search box
# dropped, its type index kept as plain links, and the result wrapped
# in a styled div so the repository skin frames it. The doc-comment
# content itself is untouched.
#
# `build-docs` must run before the final `shards build`, or the binary
# ships without docs and /docs says so.
module CrystalRobots::Web
  module Docs
    DIR = "docs-api"

    # path relative to DIR => file content, captured when the binary
    # was built. search-index.js and index.json are left out: they
    # only back crystal-docs' own search box, which the page transform
    # below drops.
    FILES = {% begin %}
      {
    ...
    # Styles for the extracted content, inline because Fossil's policy
    # allows inline styles and forbids scripts. Scoped under .crystal-docs.
    STYLE = <<-CSS
      <style>
    ...
    # A generated page transformed for the Fossil chrome, or nil.
    # Returns the page title and the HTML fragment for a wrapper div.
    def self.page(path : String) : {String, String}?
      ...
      # belt and braces: the generator is trusted, but strip anything
      # that would still run script if that ever changed.
      body = body.gsub(/\s+on\w+\s*=\s*"[^"]*"/i, "")
      body = body.gsub(/\s+on\w+\s*=\s*'[^']*'/i, "")
      body = body.gsub(/(href|src)(\s*=\s*")javascript:[^"]*(")/i, "\\1\\2#\\3")
      body = body.gsub(/(href|src)(\s*=\s*')javascript:[^']*(')/i, "\\1\\2#\\3")
      # relative links inside the generated site keep working because
      # the page keeps its place in the tree; only absolute-root links
      # would not.
      {title, STYLE + "<div class=\"crystal-docs\">" + body + "</div>"}
    end

    # Runs `crystal docs` over the CLI entry point (which requires
    # every compiler, battle and web module transitively, so crystal
    # docs crawls the whole tree from one starting point) into DIR.
    # Called by the `build-docs` CLI subcommand.
    def self.build(version : String) : Bool
      args = ["docs", "-o", DIR, "--project-name", "crystal-robots", "--project-version", version,
              "--canonical-base-url", "/ext/crystal-robots/docs/", "src/cli.cr"]
