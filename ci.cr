...
if with_wasmer
  ...
  # crystal spec -Dwasmer needs WASMER_DIR in its own process's
  # environment (the wasmer-crystal shard reads it to find libwasmer.a
  # at link time), not just the installer subprocess's env above -- a
  # clean environment with no ~/.wasmer otherwise fails at link time
  # with undefined wasm_* references.
