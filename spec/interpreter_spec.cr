...
describe Interpreter do
  ...
  it "a standalone cycle limit raises Interpreter::StepLimit instead of looping forever" do
    program = CrystalRobots::Compiler::Parser.new("global(i, 0)\nmain(\"Loop\") do\nwhile true\ni += 1\nend\nend\n").program
    host = CrystalRobots::Compiler::NullHost.new
    interpreter = Interpreter.new(program, host, limit: 500_i64)
    expect_raises(Interpreter::StepLimit) { interpreter.run }
    interpreter.cycles.should be >= 500
    interpreter.cycles.should be < 1000 # raises soon after crossing the limit, not thousands of cycles later
  end
