...
Spec suite: 101 examples green with `-Dwasmer`; `crystal run ci.cr -- --with-wasmer` green end to end; `crystal run ci.cr -- --with-wasm32` green end to end (Phase 5b-1).
...
1. **Browser-hosted WASM compiler (priority).** `src/browser.cr` exports
   ...
   Battle in the page is explicitly out of scope for this unit (next).
