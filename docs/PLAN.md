...
| 2026-09-20 | Phase 6 unit done (ticket db7474e899): `site/editor` (plain textarea, no framework, same CSP as the rest of `site/`) forks any bundled example or saved wiki robot, checks it live on a 500 ms debounce against a new non-raising `crd_check`/`Browser.check` (`src/browser.cr`, reusing `Parser.try_parse` from Phase 5b-2) that reports a parse error or checker problems as `{message, line, col}` instead of trapping, rendered beside a line-number gutter in the same plain wording `derivation_section` uses server-side; Fight feeds the buffer into the existing `crd_battle_run` and renders the same SVG replay. `RobotAPI.cheat_sheet_json` (`src/web/robot_api.cr`) exports the served panel's own `GROUPS`/`ENTRIES` as JSON, written to `site/cheat-sheet.json` by `scripts/build_cheat_sheet.cr`, a new `ci.cr --with-wasm32` step — one Crystal source of truth for both surfaces. A new `/wiki-robots.json` route (`src/web/cgi.cr`, gated like `/wiki/<name>`) lets the editor's picker offer saved robots once it is reachable from the same origin as this server, via an explicit `?wiki=<base>` query parameter the page never guesses (no CORS, no credentialed cross-origin fetch); the overview links to the GitHub Pages editor without one today. Every default (copy-and-change, debounce length, cheat-sheet always open) is a named, one-line switch, listed in "Phase 6 correction and expansion" above, waiting on the still-unanswered "Robot Builder ideas" wiki page. |

| 2026-09-20 | Phase 6 unit done (ticket a83cd0a90f, battlefield-visual upgrades bullet 4 above, robot skins out of scope for this unit): the maintainer's complaint that "cannon fire and impacts fall between samples" traced to two things — `Field`'s frame log downsampled at a fixed interval with no regard for what happened, and the in-page battle handed that thin log to one SMIL animation as its only playback mechanism, the same path the served `/battle` page uses. Fix, in `Field` itself (`src/battle/field.cr`, shared by both the fiber-based `run` and the non-fiber `run_stepwise` in `src/battle/step_robot.cr`, so native and wasm32 get it identically): an `@event_pending` flag, set by `cannon` (a shot fired), `scan` (a hit), and `move_robots`/`move_missiles` (damage dealt, a death, a missile impact), read and cleared once per motion update — that update's frame is recorded whenever the flag is set, in addition to the fixed downsample interval a frame budget (`max_frames`) still governs for quiet stretches. An event is *never* subject to the budget; only the quiet-stretch sampling rate is. `Browser.battle_run` (`src/browser.cr`) now accepts a request `max_frames` (`Browser::BATTLE_FRAMES_MIN`/`_DEFAULT`/`_MAX` = 200/20,000/60,000 — clamped to at least 200 so a page-supplied 0 cannot divide by zero, since a `raise` on that path traps the whole wasm32 module) — the served page and `bin/crystal-robots` keep their existing, much smaller defaults (`ANIM_FRAMES` 400, `Field`'s own default 200) since they still build one SMIL keyframe per frame; the in-page battle and the editor's Fight preview both request the two-orders-of-magnitude-larger default instead. Building that many SMIL `<animate>` keyframes would itself be the slow, unresponsive rendering this unit exists to fix, so `crd_battle_run` no longer renders `Battle.svg_animation` for the in-page battle at all: it returns a static SVG "skeleton" (`Battle.svg_skeleton`, `src/battle/svg_replay.cr` — the same shapes, at frame 0, tagged with stable ids, no `<animate>` elements) plus the match's per-cycle frame log packed as a compact binary buffer (`Browser.encode_frames`, layout documented in `src/browser.cr`: little-endian, a fixed-size record per robot and per missile slot at every frame, so a reader can index frame `i` directly instead of parsing a variable-length record — tens of thousands of small JSON objects would have been exactly the transfer cost this format avoids). `site/battle-playback.js` (new, no framework, no CDN) decodes that buffer and drives the skeleton's ids in `requestAnimationFrame` at 60 fps: position and heading interpolate linearly between the two bracketing recorded frames (shortest-path for heading, so a turn through 0° does not spin the long way), everything else (scan, cannon, fired, missiles) steps discretely to the last frame reached, matching `svg_animation`'s own linear/discrete split. Play/pause, a speed control in cycles per second, and a scrubber are plain DOM controls the module builds itself. `site/battle.js` and `site/editor/editor.js` both switched from inserting `result.svg` to `mountBattlePlayback(container, result, cps)`, stopping the previous match's animation loop before starting the next; `svg_animation` itself is untouched and stays the served `/battle` page's own renderer (it also benefits from event-guaranteed frames without any change on its side, since it draws from the same `Field`). `crystal run ci.cr` and `-- --with-wasm32` both green (see below); `spec/battle_spec.cr` proves a shooting robot's every impact gets a recorded frame under both `run` and `run_stepwise` at a `max_frames` of 2, and that a fatal blow's frame is never dropped either; `spec/browser_spec.cr` (new, calls `CrystalRobots::Browser` natively — nothing in `src/browser.cr` is wasm32-specific in Crystal source, only the cross-compilation target is, so this needs neither the compiled `.wasm` nor `node`) proves `frame_count` scales with the requested budget, the binary log's own header agrees with the JSON report, a non-positive `max_frames` is clamped instead of crashing, and the same event guarantee holds through the actual JSON-request/binary-response bridge; `spec/wasm32_spec.cr`'s differential battle case now compares `frame_count`/`robot_count`/`skeleton` against a native `run_stepwise` run at a fixed `max_frames` instead of the retired `svg` field. Manually verified end to end in headless Chromium on this host (driven over raw CDP with a Node script, `--no-sandbox` under this root-only environment; equivalent to what Playwright's `browser_*` tools would drive): loaded `site/index.html` from a local static server, ran rabbit vs. rabbit — neither ever fires, so nothing dies before the cycle limit — at `limit=500000`, got back 500,010 cycles and 33,336 recorded frames (the large in-page budget, `MAX_FRAMES` in `site/battle.js`, working as intended — 83x the served page's 400-frame cap), and sampled `requestAnimationFrame` timing for 4 seconds during playback: 239 consecutive frame deltas, mean 16.67 ms (60.00 fps), max 16.80 ms, zero deltas over a 60fps-frame-plus-slack threshold — a steady 60 fps with no dropped frames, no console errors. A large backward scrub (rebuilding all three robots' trails across ~30,000 frames) cost 0.2 ms in the input handler itself and produced exactly one slightly-long animation frame on the next tick (the one-time trail-rebuild cost), then playback returned to steady 60 fps — an acceptable one-frame UI-seek cost, not a dropped frame during ordinary playback. |

Spec suite: 101 examples green with `-Dwasmer`; `crystal run ci.cr -- --with-wasmer` green end to end; `crystal run ci.cr -- --with-wasm32` green end to end (157 examples, 0 failures, 0 errors, 10 pending as of the Phase 6 battlefield-visuals unit — the wasmer-gated cases; `crystal run ci.cr` plain is 157 examples, 0 failures, 17 pending).
...
**Item 6 done, 2026-09-20 (ticket db7474e899).** The wiki page's questions
are still unanswered (`Robot Builder ideas`'s "Your ideas" section is
empty), so this unit builds the plan's own stated defaults and keeps each
one a small, named switch so an answer can flip it later without a
redesign:

- **Copy-and-change, not a blank page**: `site/editor` forks the picked
  robot's source into the buffer outright (`forkSelected` in
  `site/editor/editor.js`). Switch: nothing to flip structurally, but a
  "start blank" option is one more `<option>` and an `if` away if the
  answer prefers it.
- **Checked while typing, after a short pause**: `crd_check` (parse via
  `Parser.try_parse`, then `Checker.check` -- never raising, unlike
  `crd_run`) runs on a debounce, `DEBOUNCE_MS = 500` in `editor.js`.
  Switch: one named constant.
- **One cheat-sheet, always visible**: `RobotAPI.cheat_sheet_json`
  (`src/web/robot_api.cr`) exports the exact `GROUPS`/`ENTRIES` the served
  `/parse` and `/battle` pages' panel already renders, so the editor's
  page and the served panel read the same source of truth and can never
  drift; `scripts/build_cheat_sheet.cr` writes `site/cheat-sheet.json` as
  a `ci.cr --with-wasm32` step, alongside `site/robots.json`. It renders
  open, not inside a `<details>`, per "always visible"; wrapping each
  group in a `<details open>` is a one-line switch if a collapsed default
  turns out to read better.
- **Fight from the editor**: the buffer is fed to the same
  `crd_battle_run` (`Browser.battle_run`) `site/battle.js` already calls,
  against up to three opponents picked the same way; the SVG replay is
  the same `Battle.svg_animation`, so there is exactly one battle engine
  and one renderer across the compiler page, the battle page and the
  editor.
- **Errors beside the line**: a new `crd_check`/`Browser.check` export
  (`src/browser.cr`) uses the non-raising `Parser.try_parse` (already
  built for Phase 5b-2's battle isolation) and returns structured
  `{message, line, col}` instead of `crd_run`'s trap-on-bad-source
  behavior, so a mid-typing syntax error never takes the module down;
  `site/editor/editor.js` renders a line-number gutter beside the
  textarea (synced scroll) and highlights the reported line, with the
  same plain wording `derivation_section` already uses server-side
  ("Parse error: ... at line:col", "... at line:col" per problem,
  "Checks passed: ...").
- **Saved wiki robots in the picker, only where Fossil is reachable**: a
  new `/wiki-robots.json` route (`src/web/cgi.cr`, gated like
  `/wiki/<name>`: login plus wiki-read) lists every saved robot as JSON.
  `site/editor` is a GitHub Pages static page with no Fossil behind it,
  so its picker only asks for this when given an explicit `?wiki=<base>`
  query parameter (no guessing, no CORS, no credentialed cross-origin
  fetch); without one it offers the bundled examples only. **Open switch,
  not decided here:** making `site/editor` reachable from the same origin
  as the served CGI app (so a same-origin link with `?wiki=` actually
  resolves) is a deploy-topology decision -- e.g. a second static-file
  extroot entry alongside `scripts/install_extroot.sh`'s existing binary
  symlink -- left to trunk-ops, same as the rest of Phase 5's deploy
  story (section 7, item 2 above). The overview page links to the
  GitHub Pages editor today (`CGI::EDITOR_URL`) without the parameter.

Proof: `spec/robot_api_spec.cr` checks `cheat_sheet_json` structurally
against `RobotAPI::GROUPS`/`ENTRIES` and `Interpreter.builtin_names`, the
same identity `spec/web_cgi_spec.cr`'s panel spec already guards;
`spec/wasm32_spec.cr` adds a byte-identical check of the *generated*
`site/cheat-sheet.json` against `RobotAPI.cheat_sheet_json` computed
fresh, and a case that edits `examples/sniper.cr`'s source and checks it
clean through `crd_check`, proving an edited example round-trips through
the module, not just an untouched one; `spec/web_cgi_spec.cr` covers
`/wiki-robots.json`'s listing and its login/wiki-read gate.
`crystal run ci.cr` and `crystal run ci.cr -- --with-wasm32` both green
(counts in the session report on ticket db7474e899's thread).
